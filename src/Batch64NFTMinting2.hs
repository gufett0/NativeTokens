...
module Batch64NFTMinting2 where
...
mint nparams = do
    let rAddress = npAddress nparams
    opkh  <- Contract.ownPaymentPubKeyHash
    utxos <- utxosAt (pubKeyHashAddress opkh Nothing)

    case Map.keys utxos of
        []       -> Contract.logError @String "no utxo found"
        oref : _ -> do
            let pkh     = toPubKeyHash rAddress
            let tname   = npTokenName nparams
            let val     = Value.singleton (curSymbol oref tname) tname 1
            let withAda   = Ada.lovelaceValueOf 5000000 <> val
                lookups = Constraints.mintingPolicy (policy oref tname) <> Constraints.unspentOutputs utxos
                tx      = Constraints.mustMintValue val <>
                          Constraints.mustSpendPubKeyOutput oref <>
                          Constraints.mustPayToPubKey  (PaymentPubKeyHash $ fromJust pkh) withAda
